import { Bell, MapPin, Search, Star, ChevronRight, Heart, Clock } from "lucide-react";
import {
  greeting,
  quickActions,
  upcoming,
      ...
      <div className="px-5 pb-6 pt-2">
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            <span className="flex items-center gap-1 rounded-full bg-white px-2 py-1 text-[10px] text-wcc-slate shadow-sm">
              <MapPin size={10} /> {greeting.location}
            </span>
            <button className="relative grid h-9 w-9 place-items-center rounded-2xl bg-white shadow-sm">
              <Bell size={14} className="text-wcc-green-deep" />
        ...
        <div className="mt-4 rounded-3xl bg-[#F4EADB] p-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="font-sora text-[18px] font-bold text-wcc-green-deep">
                {greeting.name} 👋
              </p>
            </div>
          </div>
          <p className="mt-3 font-sora text-[12px] leading-snug text-wcc-green-deep">
            {greeting.nudge}
          </p>
        ...
        <div className="mt-6 rounded-3xl bg-white p-4 shadow-sm">
          <div className="flex items-center justify-between">
            ...
            <span className="flex items-center gap-1 text-[10px] text-[#E07856]">
              <Clock size={10} /> {upcoming.countdown}
              <Heart size={11} className="ml-1 fill-[#E07856] text-[#E07856]" />
            </span>
          </div>
          <div className="mt-2 flex items-center gap-3">
            <img src={upcoming.avatar} alt="" className="h-14 w-14 rounded-2xl object-cover" />
          ...
          <div className="flex gap-2 overflow-x-auto pb-1">
            {specialties.map((s) => {
              ...
              return (
                <span
                  key={s.label}
                  className="flex shrink-0 items-center gap-1 rounded-full bg-white px-3 py-1.5 text-[11px] text-wcc-ink shadow-sm"
                >
        ...
        <div className="mt-6 space-y-3">
          {[pharmacy, lab].map((s) => (
            <div key={s.title} className="overflow-hidden rounded-3xl bg-white shadow-sm">
              <div className="relative">
                <img src={s.image} alt="" className="h-28 w-full object-cover" />
                <span className="absolute right-3 top-3 rounded-full bg-white/90 px-2 py-0.5 text-[10px] font-bold text-wcc-green-deep">
                  {s.tag}
                </span>
              </div>
            ...
            {tips.map((t) => (
              ...
              >
                ...
                <div className="p-3">
                  ...
                  <p className="mt-1 text-[11px] font-medium leading-tight text-wcc-ink">
                    {t.title}
                  </p>
                </div>
              </div>
